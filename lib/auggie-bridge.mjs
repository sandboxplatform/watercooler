/**
 * Auggie Bridge — ESM version for the published package's server
 * (server.prod.mjs), which cannot import TypeScript.
 *
 * Emulates the gateway protocol but delegates to the `auggie` CLI.
 *
 * A hand-kept mirror of a slice of `lib/cli-bridge.ts` — it named
 * `auggie-bridge.ts`, which has not existed since the bridge learned about
 * more than one CLI. Two copies of the run loop is the cost of shipping two
 * servers, and shipping one instead is the fix when it matters; until then,
 * a change to the run loop over there may need one here.
 */
