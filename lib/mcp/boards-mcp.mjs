 ...
 *
 * No shebang, deliberately: the bridge spawns this as `node <path>`, so one
 * was never used, and it made the file unimportable — Node strips a shebang
 * from an entry point, but Vitest wraps the module and leaves the `#!` where
 * V8 rejects it. The tests below import this, which is what the guard at the
 * bottom is for.
 *
