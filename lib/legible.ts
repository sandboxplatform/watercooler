 ...
 * handset that is the least zoom a place opens at — half size. Everything the game letters
 ...
 * fits the lobby at 0.88), and a handset — opened at half zoom by a viewport
 * nothing like that shape — doubles. Pinched out from there it goes on
 * growing, to four times at a quarter, because standing well back is
 * exactly when a name, an egg or a mailbox wants finding.
 *
 * Capped, because the scale is applied in the world and the world has other
 * things in it. `ZOOM_MIN` is 0.25, so 4 is the most this can ever ask for
...
export const LEGIBLE_MAX_SCALE = 4;
